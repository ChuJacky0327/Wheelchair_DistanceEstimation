...
broker = '192.168.50.1'#party
port = 1883
topic_warm = 'warm'
topic_object = 'object'
topic_distance = 'distance'
client_id = f'python-mqtt-{random.randint(0, 1000)}'
def publish(client,warm,object1,distance_mq):
    msg_warm = warm
    msg_distance = distance_mq
    msg_object = object1
    result_warm = client.publish(topic_warm, msg_warm)
    result_distance = client.publish(topic_distance, msg_distance)
    result_object = client.publish(topic_object, msg_object)
    '''
    status = result[0]
    if status == 0:
        print(f"Send `{msg_warm}` to topic `{topic_warm}`")
    else:
        print(f"Failed to send message to topic {topic_warm}")
    '''
...
def run(warm,object1,distance_mq):
    client = connect_mqtt()
    publish(client,warm,object1,distance_mq)  
...
def loop_and_detect(cam,cam2, trt_yolo, conf_th, vis):
    ...
    while True:
        ...
        right_img = right_img[0:480,565:640]
        tmp = np.hstack((left_img,right_img))
        try : #person
            if right_person_cm != 0 and left_person_cm != 0 :
                ...
                cv2.putText(tmp,str(dis),(10,50),cv2.FONT_HERSHEY_SIMPLEX,1,(0,255,0),2,cv2.LINE_AA)
                ...
                if left_person_cm <300:
                    cv2.putText(tmp,('person :' + str(round(left_person_cm,2)) + 'cm , left'),(10,50),cv2.FONT_HERSHEY_SIMPLEX,1,(0,0,255),2,cv2.LINE_AA)
                    object1 = 'person'
                    distance_mq = str(round(left_person_cm,2))
                    warm = 'warn'
                else :
                    cv2.putText(tmp,('person :' + str(round(left_person_cm,2)) + 'cm , left'),(10,50),cv2.FONT_HERSHEY_SIMPLEX,1,(0,255,0),2,cv2.LINE_AA)
                    object1 = 'person'
                    distance_mq = str(round(left_person_cm,2))
                    warm = 'safe'
            elif  right_person_cm != 0 and left_person_cm == 0 :
                #print('person :' + str(right_person_cm) + 'cm , right')
                if right_person_cm <300:
                    cv2.putText(tmp,('person :' + str(round(right_person_cm,2)) + 'cm , right'),(10,50),cv2.FONT_HERSHEY_SIMPLEX,1,(0,0,255),2,cv2.LINE_AA)
                    object1 = 'person'
                    distance_mq = str(round(right_person_cm,2))
                    warm = 'warn'
                else:
                    cv2.putText(tmp,('person :' + str(round(right_person_cm,2)) + 'cm , right'),(10,50),cv2.FONT_HERSHEY_SIMPLEX,1,(0,255,0),2,cv2.LINE_AA)
                    object1 = 'person'
                    distance_mq = str(round(right_person_cm,2))
                    warm = 'safe'
            else :
